...
# defined libraries to find thesolution of the command used in this code.

# load correlation heat map utility
load(file = "correlation_heat_map.RData")
#this command is used to reload the saved dataset

# note. evcent() often produces warning about lack of convergence
# we will ignore these warnings in this demonstration program
options(warn = -1)

# defines total number of iterations used for this statistical simulations
NITER <- 100
...
# computes the adjacency matrix of the gaussian weighted graph
...
set.seed(1)
# seeding random varaite generators 

random_graph <- erdos.renyi.game(n = 50, type = "gnm", p.or.m = 100)  
pdf(file = "fig_network_random_graph.pdf", width = 5.5, height = 5.5)
# Creates Pdf Graph wih defualt width and height of 7 if not mentioned
# generates random graph with 50 nodes and 100 links
...
# use to plot X-Y graph
dev.off()
# dev,off is command to shut down the graphics device 
...
aggregate_degree <- NULL  # starting the collection of node degree values
...
# ---------------------------------
# We have example of a small-world network which has 10 nodes and 1D model (no random links)
set.seed(1)
...
# Hence, we used 1D small world problem with three cases such as nei = 1, 2, & 3 for 10 nodes  and each node has 4 links and displayed 
grpahs for each.


# Now using same 1D small world model for probability p = 0.2
...
# scale commmand is used to scle and for centering of matrix like objects  
...
#Displayed graph for node degree distrubtion  
...
# Here we compared both graphs as displayed with pdf command for small world degree data frame case with random graph degree data frame
...
# we used lattice graphics to compare degree distributions as named under fig_network model degree distrubutions pdf with predefined width as 8.5 nd height 11 and is 
displayed by pdf command in R. 
